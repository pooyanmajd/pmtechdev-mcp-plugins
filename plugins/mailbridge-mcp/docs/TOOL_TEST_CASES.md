...
**Expected:** `mail_send_reply` first presents a compact MCP form elicitation that looks like a Gmail compose review: From / To / Cc / Bcc / Subject / Reply, then a boxed Message. Ordinary values render unquoted; hostile headers and body lines JSON-encode so they cannot forge trusted labels. Only an accepted confirmation can invoke the JXA dispatcher, which replaces quoted content with the approved body, verifies Mail resolved the same recipients, and submits one attachment-free reply atomically. A mismatch fails with `SEND_TARGET_CHANGED`; an unavailable or declined form fails with `CONFIRMATION_UNAVAILABLE` or `SEND_NOT_CONFIRMED`. The result reports only that Mail accepted it for sending. No draft-send, forward-send, attachment-send, or bulk-send path exists.
