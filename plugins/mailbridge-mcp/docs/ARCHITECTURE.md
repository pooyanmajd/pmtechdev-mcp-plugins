...
`src/local-config.ts` reads and writes the local, per-user access-preferences file consulted by `src/cli.ts` at startup and by the access-preference tools. It is independent of the Mail.app/JXA trust boundary below — no Apple Events are involved — and a missing, corrupt, or unreadable file degrades to the built-in default with a stderr diagnostic rather than crashing startup. On MCP Apps hosts, `mailbridge_set_access_preferences` only prepares a short-lived exact proposal and renders `src/server/access-preferences-ui.ts` as an MCP Apps resource; it writes nothing. The proposal identifier is delivered in model-hidden result `_meta`. Only the app-visible `mailbridge_commit_access_preferences` tool can consume it after the user presses **Save access**. On other hosts, the set tool presents a native exact-scope form and commits internally only after acceptance; no app-only finalizer is registered. Hosts without either interface fail before account verification or persistence. Before every read or write, the local-config layer `lstat`s (never follows) the config directory and file: a pre-existing symlink, wrong entry type, or an entry owned by a different user is refused rather than followed, since macOS `chmod` follows a symlinked directory to its target — relevant if `XDG_CONFIG_HOME` points at a shared location. Reads also reject a file above a small fixed size before loading it, so a large corrupt or planted file cannot be read fully into memory at every startup.
...
Read-only mode blocks all mutations. Draft mode permits draft creation but not message-state changes. Full mode additionally permits read/flag state changes and deliberately retains its non-send semantics. Prompted mode permits those operations plus the two reviewed send tools, but each send fails closed unless the MCP client presents and accepts a form elicitation showing the exact sender, recipients, body, and new-message subject or reply-to subject context. The form separates reviewed fields visibly, JSON-encodes hostile values, and renders each body line with a visible marker so email content cannot forge its trusted review structure. Direct send mode requires mode and a non-empty sender allowlist to originate in the environment. Modifying operations are serialized and bounded by a queue; an uncertain send outcome is reported as `MUTATION_OUTCOME_UNKNOWN` so callers inspect Mail.app instead of blindly retrying.

Mail's public outgoing-message interface cannot provide a complete, stable attachment inventory for every draft, so Mailbridge never sends an arbitrary edited draft or forward. `mail_send_message` and `mail_send_reply` instead construct one attachment-free outgoing object from bounded, validated input and submit it within the same JXA operation. New-message subject/body and reply body are read back before submission and changed content fails closed; Mail generates the reply subject from the selected source message. Current Mail.app serializes one extra terminal ASCII space after any scripted body assignment, new message or reply; Mailbridge accepts only that exact additive transform and rejects every other body difference. Mail's resolved recipient sets must match the approved expected recipients immediately before submission. Each call requires a substantive body and literal `confirmed: true`, plus either an accepted client elicitation in prompted mode or an explicit environment sender allowlist in direct send mode. In prompted mode that flag requests the native final review and acceptance authorizes the send. In direct send mode, guidance requires the exact content to be shown and approved in chat before the flag is set. The runtime cannot enforce human comprehension outside prompted mode, so the trusted-host boundary remains material.
...
- All Mail.app automation is serialized with a bounded queue. This avoids concurrent Apple Event scans competing inside Mail.app; excess parallel work fails with `AUTOMATION_BUSY` instead of building an unbounded backlog. The queue wraps only the point where an operation actually reaches the bridge/JXA layer — mode and authorization checks run before a queue slot is acquired, so they cannot starve unrelated calls.
- Send elicitation waits (which can legitimately take minutes) are bounded by their own separate serial queue, not the automation queue. Access proposals are held in a separate bounded, ten-minute in-memory store, and card commits use a dedicated serial queue. This keeps human interaction from starving unrelated Mail.app calls while preventing unbounded proposal or write accumulation.
...
List, search, and get operations declare `readOnlyHint=true`, `destructiveHint=false`, and `openWorldHint=false`. State and draft tools declare `readOnlyHint=false`, `destructiveHint=false`, and `openWorldHint=false`. Send tools declare `readOnlyHint=false`, `destructiveHint=true`, `idempotentHint=false`, and `openWorldHint=true` because they communicate externally through Mail.app and cannot be safely retried. On MCP Apps hosts, `mailbridge_set_access_preferences` is a read-only, non-idempotent render tool because it creates a fresh ephemeral proposal but performs no durable mutation. The native-form fallback instead declares a local, destructive, idempotent replacement mutation and saves only after the host form is accepted. The app-only `mailbridge_commit_access_preferences` tool declares `readOnlyHint=false`, `destructiveHint=true`, `idempotentHint=true`, and `openWorldHint=false`: it is local and safely retryable, but replaces the complete persisted permission record. These annotations improve host UX; runtime validation, app-only visibility, and the unguessable short-lived proposal remain the consent boundary.

## Mode-scoped tool advertising

Each tool registers only in the modes that can actually use it, so a `read-only` server never advertises `mail_send_message` only to have it fail closed. This is an advertising/UX filter evaluated once at startup in `src/server/index.ts`; it does not replace the authoritative runtime checks in `src/server/service.ts` (`requireDraftsMode`/`requireStateChangeMode`/`sendAuthorization`), which remain the actual security boundary and fail closed regardless of what was advertised. The three access-preference tools are the exception, registered in every mode so a read-only server can be reconfigured without a restart-then-guess cycle; the commit tool is marked visible only to the MCP App.

## Distribution

The production build commits a reproducible ESM executable at `dist/cli.js`; the fixed dispatcher is shipped at `runtime/mailbridge.jxa.js`. Host-specific inline registrations prevent one client's path semantics from overriding another's: Codex resolves `./dist/cli.js` with the plugin root as `cwd`, Claude Code uses `CLAUDE_PLUGIN_ROOT`, and Grok uses `GROK_PLUGIN_ROOT`. All three select prompted mode so drafts work immediately while sends require client confirmation. Plugin users do not need development dependencies or a source build. Plugin assets and its skill are presentation and agent-guidance layers; runtime policy remains enforced inside the server.
