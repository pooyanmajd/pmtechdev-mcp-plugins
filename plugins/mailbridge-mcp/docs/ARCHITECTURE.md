...
- A prompted send's client-side elicitation wait (which can legitimately take minutes) is bounded by its own separate, equally-sized serial queue, not the automation queue. This keeps a pending confirmation from starving unrelated Mail.app calls (the original problem) while also bounding how many confirmations can be concurrently pending or queued — excess concurrent send attempts fail with `CONFIRMATION_BUSY` instead of accumulating unbounded pending promises and client-side prompts.
