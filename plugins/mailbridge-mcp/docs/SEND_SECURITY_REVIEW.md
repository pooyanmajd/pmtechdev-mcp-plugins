...
4. The request contains bounded recipients, a substantive body, and literal `confirmed: true`. Mailbridge reads the constructed outgoing subject/body back before submission and refuses changed content, treating one Mail.app-added terminal ASCII space as the only deterministic serialization artifact; any other additional, removed, or altered character still fails closed. Replies additionally carry the exact expected recipient sets, checked immediately before submission.
...
| Content changes after approval | No send-draft operation; outgoing object is built atomically and its subject/body are read back before submission. Verification permits only Mail.app's fixed terminal ASCII-space serialization | Mail.app/provider processing after acceptance is outside the boundary |
...
`mailbridge_set_access_preferences`'s `mode` field deliberately excludes `send`, restricted to `read-only`/`drafts`/`full`/`prompted`. A model-supplied `confirmed: true` on this tool is not an independently verified human confirmation the way MCP elicitation is for gate 2 above — it only reflects that agent guidance was followed, which a compromised or prompt-injected host cannot be assumed to do. Allowing this tool to set `send` mode would let exactly that kind of host grant itself standing, per-call-unconfirmed send authority for a future session merely by claiming `confirmed: true`, with no gate 2-equivalent check ever applied. Elevating to `send` mode therefore stays a manual, human-performed environment-variable change, consistent with gate 3 requiring a deliberately configured allowlist. Does not change the review conclusion.

Releasing the elicitation wait from the automation queue (see Architecture) removed an incidental bound on concurrent pending confirmations. A dedicated, equally-sized serial confirmation queue restores that bound independently, so a caller cannot accumulate unlimited concurrent elicitation forms, promises, or client-side prompts; excess concurrent attempts fail closed with `CONFIRMATION_BUSY` before ever reaching `confirmMailSend`.

## Addendum: 0.5.0 host-agnostic preview, account scope, and Grok

`mail_preview_outbound` is a **review surface**, not a new authority. It renders the same Gmail-style card prompted elicitation shows. Ordinary header values display unquoted so the dialog is readable; values with control characters, newlines, or header-impersonating prefixes (`From:`, `Subject:`, …) still JSON-encode. Calling preview does not authorize a send. Prompted mode still requires host form elicitation; `send` mode still requires a human-set allowlist. A model can call preview and then lie that the user approved — that residual risk is unchanged from `confirmed: true` and is why elicitation / env-only `send` remain the gates.

Unscoped `mail_search_messages` now fails with `ACCOUNT_SCOPE_REQUIRED` when more than one account is visible and no allowlist is configured. This is a confidentiality control against accidental work/personal crossover into the model, not an authorization token.

The local preferences **file schema** now rejects `send` on both read and write. A hand-edited `mode: "send"` file is ignored. Combined with the tool already refusing `send`, the only way to start in direct send mode is `MAILBRIDGE_MODE=send` plus a non-empty allowlist in the environment.
