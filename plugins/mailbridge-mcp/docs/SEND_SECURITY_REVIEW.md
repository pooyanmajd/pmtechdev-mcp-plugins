...
5. Agent guidance requires the exact sender, To/CC/BCC recipients, new-message subject or reply-to subject context, reply-all state, and complete body to be shown and explicitly approved before `confirmed` is set. Prompted mode adds a server-mediated confirmation; it does not replace this review requirement.
