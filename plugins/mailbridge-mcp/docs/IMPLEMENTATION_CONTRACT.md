...
- `mailbridge_set_access_preferences` (read-only render tool for an exact access proposal; no preliminary chat approval or caller-supplied confirmation flag)
...
- `mailbridge_set_access_preferences`: `readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=false`, `openWorldHint=false` because it only prepares an ephemeral inline review card.
