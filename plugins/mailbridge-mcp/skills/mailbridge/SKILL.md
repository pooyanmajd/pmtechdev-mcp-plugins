...
- For `CONFIRMATION_UNAVAILABLE`, explain that the current MCP client cannot present the required confirmation interface. For access preferences, use a host with MCP Apps or form elicitation. For sending, offer an editable draft or a reviewed allowlisted direct registration. Never bypass the confirmation.
